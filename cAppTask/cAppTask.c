...
static void *get_weigh_gd32_thread(void *args) 
{
    ...
    while(1)
    {
        ...
        debug_print("获取is_use = %d, uuid = %02x %02x %02x %02x %02x %02x %02x %02x, 重量 = %d\n",
                            pTag.is_use, pTag.uid[0], pTag.uid[1], pTag.uid[2], pTag.uid[3], 
		...
		//字符串形式发送
		// sprintf(sendData,"获取is_use = %d\n, uuid = %02x %02x %02x %02x %02x %02x %02x %02x\n, 重量 = %d\n",
        //                     pTag.is_use, pTag.uid[0], pTag.uid[1], pTag.uid[2], pTag.uid[3], 
        //                     pTag.uid[4], pTag.uid[5], pTag.uid[6], pTag.uid[7], pTag.weighValue);

		//udpsocket_set_weigh_uuid_data(sendData); 
        usleep(50000);
    }
}
